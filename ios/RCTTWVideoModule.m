...
- (TVIVideoFormat *)getCameraDimension:(AVCaptureDevice *) device targetSize:(CMVideoDimensions)targetSize {
    TVIVideoFormat *selectedFormat = nil;
    // Ordered from smallest to largest.
    NSOrderedSet<TVIVideoFormat *> *formats = [TVICameraSource supportedFormatsForDevice:device];
...
- (void)startCameraCapture:(NSString *)cameraType {
    ...
    CMVideoDimensions targetDimension = (CMVideoDimensions){900, 720};
    
    TVIVideoFormat *format = [self getCameraDimension:camera targetSize:targetDimension];
